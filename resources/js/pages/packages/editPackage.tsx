...
type PackageProps = {
  package: {
    ...
    location: string;
    agent_addon_price: string;
    agent_price_type: string; // "fixed" or "percentage"
    booking_start_date: string; // full ISO
    booking_end_date: string; // full ISO
    is_active: boolean;
    is_featured: boolean;
    is_refundable: boolean;
    visibility: string;
    ...
    booking_class: string;
    hotel_name: string;
    hotel_star_rating: number | null; // numeric
    hotel_checkin: string; // full ISO or empty
    hotel_checkout: string; // full ISO or empty
    activities: { id: number }[];
  };
  images: ExistingMedia[];
  allActivities: Activity[];
...
export default function EditPackage() {
  ...
  // Strip off time component so date inputs initialize correctly
  const isoToDate = (iso: string | null) => {
    return iso ? iso.split("T")[0] : "";
  };

  // Convert numeric star rating to a string for the Select
  const starToString = (rating: number | null) =>
    rating !== null && rating !== undefined ? rating.toString() : "";

  const [existingImages, setExistingImages] =
    useState<ExistingMedia[]>(existingImagesProp);
  const [deleteMedia, setDeleteMedia] = useState<number[]>([]);
  const [newImagePreviews, setNewImagePreviews] = useState<string[]>([]);
  ...
  const { data, setData, put, processing, errors, progress } = useForm({
    ...
    location: pkg.location,
    agent_addon_price: pkg.agent_addon_price,
    // **IMPORTANT**: convert numeric or existing string directly to string
    agent_price_type: pkg.agent_price_type || "",

    // Strip to "YYYY-MM-DD"
    booking_start_date: isoToDate(pkg.booking_start_date),
    booking_end_date: isoToDate(pkg.booking_end_date),
    ...
    hotel_name: pkg.hotel_name || "",
    // convert numeric to string for Select
    hotel_star_rating: starToString(pkg.hotel_star_rating),
    hotel_checkin: isoToDate(pkg.hotel_checkin),
    hotel_checkout: isoToDate(pkg.hotel_checkout),
  ...
  const handleNewImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setData("package_images", [
      ...(data.package_images as File[]),
      ...validFiles,
    ]);
    setNewImagePreviews((prev) => [...prev, ...previews]);
    e.target.value = "";
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-xs text-gray-500">
                        Include important terms and requirements for this
                        package.
                      </p>
                      {errors.terms_and_conditions && (
                        <p className="text-sm text-red-600">
